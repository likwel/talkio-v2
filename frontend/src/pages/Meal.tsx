...
/** Suivi & Collecte : projets MEAL (cadre logique, indicateurs) + formulaires d'enquête. */
export default function Meal() {
  const [params, setParams] = useSearchParams();
  const tab: Tab = params.get('tab') === 'forms' ? 'forms' : 'projets';
  const setTab = (t: Tab) => setParams(t === 'projets' ? {} : { tab: t }, { replace: true });

  return (
    <div className="flex h-full flex-col">
      <PageHeader icon={<IconAnalytics className="h-6 w-6 shrink-0 text-[var(--accent)]" />} title="Suivi & Collecte">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <div className="page max-w-8xl">{tab === 'forms' ? <FormsPanel /> : <ProjectsPanel />}</div>
      </div>
    </div>
  );
}
